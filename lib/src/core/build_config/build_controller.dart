...
final buildConfigProvider = StateNotifierProvider<BuildController, BuildGeneric>((ref)=>BuildController(ref));
...
class BuildController extends StateNotifier<BuildGeneric>{
  ...
  Future<String?> updateSourceCodeDirectory({String? sourceCode})async{
    String? result;
    if (sourceCode != null) {
      BuildConfigModel? buildConfigModel = await buildConfigDao.addBuildConfigData(BuildConfigModel(sourceCodeDirectory: sourceCode));
  ...
  fetchSourceCodeDirectory()async{
    BuildConfigModel? buildConfigModel = await buildConfigDao.readBuildConfigData();
